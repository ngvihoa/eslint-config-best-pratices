...
- `import-x/order`: enforces type, built-in, external package, internal alias (`@/`, `~/`), parent, and sibling/index groups with blank lines between them. Misplaced side-effect imports are reported too.
- `best-practices/sort-imports`: an auto-fixable custom rule that works with `import-x/order` to sort imports by group and then by single-line named, multiline named, and default/side-effect form. Multiline named specifiers are sorted by descending length.
